...
/*	var actualPageFileName = location.href.split("/").slice(-1).toString();
	...
	function addNavbarElements() {
		...
		//	$.getJSON(getDefaultLanguageDirectory() + jsonFileName, function(data) { //se passa nuns sitios, tem de passar nos outros, pq é difcil nao passar sem influenciar
				...
				defaultTitle = addLastInfoToShow(data);
	...
	function addLastInfoToShow(dataLastInfoToShow) {
		var toAppend;
		
		if(dataLastInfoToShow.imageTitle)
			toAppend = "<img style=\"display:inline; height: 35px\" src=\"" + topPathnamePrefix + dataLastInfoToShow.imageTitle + "\" alt=\"" + dataLastInfoToShow.title + "\">";
		else
			toAppend = dataLastInfoToShow.title;
		
		if(dataLastInfoToShow.titleLink)
			toAppend = "<a href=\"" + dataLastInfoToShow.titleLink + "\">" + toAppend + "</a>";
		console.log(toAppend);
		
		return ((dataLastInfoToShow.lastInfoToShow)? addLastInfoToShow(dataLastInfoToShow.lastInfoToShow) + " > " : "") + toAppend;
	}
		...
		$(document).on("languageChange", function(e, langId, jsonDirectory, isDefaultLanguage) {
			...
			if(isDefaultLanguage) {
				...
				//$.getJSON(jsonDirectory + jsonFileName, function(data) { 
					var htmlToNavabarTitle;
					
					//se existir um atributo imageTitle, então vai ser a imagem indicada por esse link o navbar brand.
					if(data.imageTitle)
						htmlToNavabarTitle = "<img style=\"display:inline; height: 35px\" src=\"" + data.imageTitle + "\" alt=\"" + data.title + "\">";
					else
						htmlToNavabarTitle = data.title;
					console.log(data);
					
					//se existir um atributo lastInfoToShow, então a mesma vai ser adicionada atrás de defaultTitle.
					if(data.lastInfoToShow) {
						var toAppend;
						
						if(data.lastInfoToShow.imageTitle)
							toAppend = "<img style=\"display:inline; height: 35px\" src=\"" + topPathnamePrefix + data.lastInfoToShow.imageTitle + "\" alt=\"" + data.lastInfoToShow.title + "\">";
						else
							toAppend = data.lastInfoToShow.title ;
						
						toAppend += " > ";
						
						htmlToNavabarTitle = toAppend + htmlToNavabarTitle;
						
					}
					
					
					$("#navbar-title").html(htmlToNavabarTitle);
					
					//isto sai, porque isto não vai mudar. titleLink não vai mais estar no json a não ser da linguagem default
					//if(data.titleLink)
						//$("#navbar-title").attr("href", data.titleLink);
