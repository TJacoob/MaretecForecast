...
//falta a definição de erros...
	...
	if( navigator.userAgent.toLowerCase().indexOf('firefox') > -1) { // É firefox??? 	
				//Analisar qual secção deve ser apresentado
		var sectionToPresent = getParameterByName("section");
		if(sectionToPresent)
			$("[navbarelementid=\""+ sectionToPresent +"\"] a").click(); //simular click.
		else
			$("[navbarelementid=\""+ getDefaultSectionDomain() +"\"] a").click();
		//setInitialLanguage();
	}
	...
	*/
	...
	function actionSelectedStations(){
		...
		for (var i = 0; i < selectedStations.length; i++) {
			...
			divSelSta += '<td><a href="#" onclick="removeStation(\'' + stationName + '\');"><span class="glyphicon glyphicon-remove" style="color:red"></span></a></td>'
...
//console.log(indeces)
	...
	console.log("segundo");
		...
		loadStationLayers(getDefaultLanguageDirectory(), true);
